            ...
            if collet_start_time_dt and isinstance(collet_start_time_dt, datetime):
                ...
                if self.THROUGHPUT_METRICS_FOR_SUMMING.issubset(set(self.all_subscribed_metrics)):
                    ...
                    if dl_values and isinstance(dl_values, list) and len(dl_values) > 0:
                        ...
                        # ***** CHANGE HERE *****
                        except (ValueError, TypeError): self.logger.debug(f"THP Agg: Could not convert DRB.UEThpDl value '{dl_values[0]}' for {e2_agent_id} at {collet_start_time_dt}")
                    ...
                    if ul_values and isinstance(ul_values, list) and len(ul_values) > 0:
                        ...
                        # ***** CHANGE HERE *****
                        except (ValueError, TypeError): self.logger.debug(f"THP Agg: Could not convert DRB.UEThpUl value '{ul_values[0]}' for {e2_agent_id} at {collet_start_time_dt}")
                ...
                for stat_metric_name in self.STATISTICAL_METRICS:
                    if stat_metric_name in self.all_subscribed_metrics:
                        ...
                        if metric_val_list and isinstance(metric_val_list, list) and len(metric_val_list) > 0:
                            # Check if the value is None before attempting conversion
                            metric_raw_value = metric_val_list[0]
                            if metric_raw_value is not None:
                                try:
                                    collected_stat_values[stat_metric_name] = float(metric_raw_value)
                                # ***** CHANGE HERE *****
                                except (ValueError, TypeError):
                                    self.logger.debug(f"STAT Agg: Could not convert {stat_metric_name} value '{metric_raw_value}' for {e2_agent_id} at {collet_start_time_dt}")
                            else:
                                # Value is None, log at debug if needed, but don't try to convert
                                self.logger.debug(f"STAT Agg: Metric {stat_metric_name} value is None for {e2_agent_id} at {collet_start_time_dt}. Skipping for stats.")
                        # else:
                ...
                with self.aggregation_lock:
                    ...
                    if len(self.subscribed_e2_node_ids) > 0 and \
                       len(agg_window["reported_nodes"]) == len(self.subscribed_e2_node_ids):
                        ...
                        # ... (rest of the INFO logging for aggregated results remains the same) ...
                        ...
                        for qos, metrics_for_qos in sorted(agg_window["qos_metric_values"].items()):
                            ...
                            for metric_name, values in sorted(metrics_for_qos.items()):
                                if values: # Ensure there are values to calculate stats
                                    val_min = min(values)
                                    val_max = max(values)
                        ...
                        # Print to stdout (optional, can be removed if log file is primary)
                        ...
                        # You might want to add a summary of key stats to stdout if needed
                        # for qos, metrics_for_qos in sorted(agg_window["qos_metric_values"].items()):
                        #     print(f"    QoS Class {qos}:")
                        #     for metric_name, values in sorted(metrics_for_qos.items()):
                        #         if values: print(f"      {metric_name}: Avg={(sum(values) / len(values)):.2f} (N={len(values)})")
...
if __name__ == '__main__':
    ...
    if args.kpm_report_style == 1:
        ...
        for sm in {"DRB.RlcSduDelayDl", "DRB.AirIfDelayUl", "DRB.RlcDelayUl", "DRB.RlcPacketDropRateDl"}: # Combined STATISTICAL_METRICS
